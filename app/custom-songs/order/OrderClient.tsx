...
function normalizeInitialPkg(pkg?: string): PackageChoice | undefined {
  if (!pkg) return undefined;
  const v = pkg.toLowerCase().trim();

  // allow simple URLs like ?pkg=video or ?pkg=audio
  if (v === "video") return "video";
  if (v === "audio") return "song_audio";

  // allow exact ids too
  if (PACKAGES.some((p) => p.id === v)) return v as PackageChoice;

  return undefined;
}

export default function OrderClient({ initialPkg }: { initialPkg?: string }) {
  ...
  // Apply ?pkg=... once (only if user hasn't picked yet)
  useEffect(() => {
    const desired = normalizeInitialPkg(initialPkg);
    if (!desired) return;

    setForm((prev) => {
      // if they already selected something, don't override
      if (prev.packageChoice) return prev;
      const next = { ...prev, packageChoice: desired };
      saveOrder(next);
      return next;
    });
    // keep them on step 1 (package page)
  }, [initialPkg]);
